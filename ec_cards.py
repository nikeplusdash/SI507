...
class Deck:
    ...
    def deal(self, num_hands, num_cards):
        '''returns a list of hands from the Deck
        
        self.cards is modified in place. Deck size will be reduced
        by num_hands * num_cards
        Parameters  
        -------------------
        num_hands: int
        ...
        -------
        list
            a list of hands, each hand containing num_cards cards
        '''
        hands = []
        if num_hands * num_cards > len(self.cards) or num_cards == -1:
            num_cards = 52 // num_hands
        for _ in range(num_hands):
            hand_cards = hand.Hand()
            for _ in range(num_cards):
                hand_cards.add_card(self.deal_card())
